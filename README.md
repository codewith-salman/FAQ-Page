<div id="top"></div>

<br />
<div align="center">

<!-- 🔰 BADGES -->
![GitHub repo size](https://img.shields.io/github/repo-size/codewith-salman/faq-page)
![GitHub stars](https://img.shields.io/github/stars/codewith-salman/faq-page?style=social)
![GitHub forks](https://img.shields.io/github/forks/codewith-salman/faq-page?style=social)

[![Live Demo](https://img.shields.io/badge/Live%20Demo-View%20Project-brightgreen?style=for-the-badge&logo=vercel)](https://faq-page-drab.vercel.app/)

[![GitHub Follow](https://img.shields.io/github/followers/codewith-salman?style=social)](https://github.com/codewith-salman)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Muhammad%20Salman%20Arshad-blue?style=social&logo=linkedin)](https://www.linkedin.com/in/muhammad-salmanarshad/)
[![Figma](https://img.shields.io/badge/Figma-codewithsalman-black?style=social&logo=figma)](https://www.figma.com/@codewithsalman)
[![Email](https://img.shields.io/badge/Email-msalmanwebdev%40gmail.com-red?style=social&logo=gmail)](mailto:msalmanwebdev@gmail.com)

<br />

<h1 align="center">❓ FAQ Page</h1>

A clean, modern, and interactive **Frequently Asked Questions (FAQ) Page** built using  
**HTML, CSS, and JavaScript**.  
Designed to present common questions and answers in a clear, accessible, and user-friendly layout.

<a href="https://faq-page-drab.vercel.app/"><strong>➥ Live Demo</strong></a>

</div>

---

<!-- TABLE OF CONTENTS -->
<details>
  <summary>📑 Table of Contents</summary>
  <ol>
    <li><a href="#about-the-project">About The Project</a></li>
    <li><a href="#features">Features</a></li>
    <li><a href="#built-with">Built With</a></li>
    <li><a href="#contact">Contact</a></li>
  </ol>
</details>

---

## 📖 About The Project

<img src="FAQ%20Page/assets/images/desktop.png" alt="FAQ Page Desktop View" width="100%" />

The **FAQ Page** is a simple yet effective UI component created to help users quickly find answers to commonly asked questions.  
It uses an accordion-style layout that allows users to expand and collapse answers, keeping the interface clean and easy to navigate.

This project demonstrates front-end skills such as **interactive UI behavior, clean layout structuring, responsive design, and smooth transitions** using HTML, CSS, and JavaScript.  
It’s an ideal component for websites like **product pages, service websites, SaaS platforms, and support sections**.

Ideal for:
- Website help & support sections  
- Product and SaaS FAQs  
- Service-based websites  
- UI/UX practice components  
- Portfolio demonstrations  

---

## ✨ Features

- Interactive accordion-style FAQ layout  
- Smooth open/close animations  
- Clean and readable typography  
- Fully responsive design  
- Minimal and modern UI  
- Easy to customize questions and answers  

---

## 🛠️ Built With

- **HTML5**  
- **CSS3**  
- **JavaScript (Vanilla)**  

---

## 📬 Contact

**Muhammad Salman Arshad**

- 💼 **LinkedIn:** [Muhammad Salman Arshad](https://www.linkedin.com/in/muhammad-salmanarshad/)  
- 🎨 **Figma:** [codewithsalman](https://www.figma.com/@codewithsalman)  
- 📧 **Email:** [msalmanwebdev@gmail.com](mailto:msalmanwebdev@gmail.com)

<p align="right">(<a href="#top">back to top</a>)</p>
